...
public class DossierControllerCollabo implements Initializable {
    private static final Logger LOGGER = LoggerFactory.getLogger(DossierControllerCollabo.class);
    ...
    @FXML
    private TableView<DossierCollabo> dossierTableView;
    @FXML
    private TableColumn<DossierCollabo, String> id_Dossier;
    @FXML
    private TableColumn<DossierCollabo, String> nomDossier;
    @FXML
    private TableColumn<DossierCollabo, String> nomClient;
    @FXML
    private TableColumn<DossierCollabo, String> nomAvocat;
    @FXML
    private TableColumn<DossierCollabo, Date> date_dossier;
    @FXML
    private TableColumn<DossierCollabo, String> editCol;
    DossierCollabo dossier = null ;
    ObservableList<DossierCollabo> dossiers = FXCollections.observableArrayList();
    ...
    private void loadDate() {
        ...
        nomClient.setCellValueFactory(new PropertyValueFactory<>("nomDossier"));
        nomDossier.setCellValueFactory(new PropertyValueFactory<>("nomAvocat"));
        nomAvocat.setCellValueFactory(new PropertyValueFactory<>("nomClient"));
        date_dossier.setCellValueFactory(new PropertyValueFactory<>("date_dossier"));

        Callback<TableColumn<DossierCollabo, String>, TableCell<DossierCollabo, String>> cellFoctory = (TableColumn<DossierCollabo, String> param) -> {
            final TableCell<DossierCollabo, String> cell = new TableCell<>() {
                @Override
    ...
    private void refreshTable() {
        try {
            ...
            String i = null;
            String info = userPreferences.get("User", i);
            String query = "SELECT * FROM `dossier` WHERE `nomAvocat` =?";
            PreparedStatement preparedStatement = cnx.prepareStatement(query);
            preparedStatement.setString(1, info);
            result = preparedStatement.executeQuery();

            while (result.next()){
                dossiers.add(new DossierCollabo(
