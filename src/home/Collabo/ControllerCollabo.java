...
public class ControllerCollabo implements Initializable {
    private static final Logger LOGGER = LoggerFactory.getLogger(ControllerCollabo.class);
    ...
    @FXML
    private TableView<home.Collabo.accueilCollabo> accueilTableView;
    @FXML
    private TableColumn<accueilCollabo, String> id_Dossier  ;
    @FXML
    private TableColumn<accueilCollabo, String> nomDossier;
    @FXML
    private TableColumn<accueilCollabo, String> nomClient;
    @FXML
    private TableColumn<accueilCollabo, String> nomAvocat;

    accueilCollabo accueil = null ;

    ObservableList<accueilCollabo> accueilsList = FXCollections.observableArrayList();
    @Override
    public void initialize(URL location, ResourceBundle resources) {
        ...
        try {
            ...
            String i = null;
            String info = userPreferences.get("User", i);
            String query = "SELECT COUNT(*) AS total FROM `dossier` WHERE `nomAvocat`=?";
            PreparedStatement preparedStatement = cnx.prepareStatement(query);
            preparedStatement.setString(1, info);
            result = preparedStatement.executeQuery();
            result.next();
        ...
        try {
            ...
            String i = null;
            String info = userPreferences.get("User", i);
            String query = "SELECT COUNT(*) AS total FROM `agenda`WHERE`nomAvocat`=?";
            PreparedStatement preparedStatement = cnx.prepareStatement(query);
            preparedStatement.setString(1, info);
            result = preparedStatement.executeQuery();
            result.next();
        ...
        try {
            ...
            String i = null;
            String info = userPreferences.get("User", i);
            String query = "SELECT COUNT(*) AS total FROM `tache`WHERE `nomAvocat`=?";
            PreparedStatement preparedStatement = cnx.prepareStatement(query);
            preparedStatement.setString(1, info);
            result = preparedStatement.executeQuery();
            result.next();
    ...
    private void loadDate() {
        ...
        Callback<TableColumn<accueilCollabo, String>, TableCell<accueilCollabo, String>> cellFoctory = (TableColumn<accueilCollabo, String> param) -> {
            final TableCell<accueilCollabo, String> cell = new TableCell<>() {
                @Override
    ...
    private void refreshTable() {
        try {
            ...
            while (result.next()){
                accueilsList.add(new accueilCollabo(
